...
- Do not sync live model metadata unless the matched command is `sync` and the user confirms a recency boundary.
...
| `implement` | `implement`, `build`, `wire`, `integrate`, `apply to project` | Modify a host project to use a selected provider path |
...
2. Choose the provider skill before choosing models or profiles.
...
2. Identify `RequestKind`, provider, region, `ConnectionProfileKey`, `ApiSurface`, `ResolvedBaseUrl`, `ResolvedRequestUrl`, and `API Model`.
...
```text
implement OpenAI chat and image generation settings UI using selected catalog rows only
```
...
LLM/_shared/recency-window-policy.md
LLM/skill-llm-<provider>/references/model-sync.md
```

Required steps:

1. Confirm one recency boundary with the user, proposing `6 months` by default.
2. Convert the boundary into one absolute cutoff date.
3. Review official provider docs live.
4. Update `model-catalog.md`, `pricing-matrix.md`, `request-urls.md`, and `capability-matrix.md` together when relevant.
5. Keep unverified fields as `unknown`.

Stop if official docs are inaccessible, the recency boundary is unconfirmed, or a field cannot be verified from official sources.

Example:

```text
sync Gemini models with a 6 month recency boundary
```
...
5. Check that selected catalog rows have capability and pricing rows.
6. Check that logging fields match `LLM/_shared/logging-fields.md`.
7. Run available non-destructive validation commands.
