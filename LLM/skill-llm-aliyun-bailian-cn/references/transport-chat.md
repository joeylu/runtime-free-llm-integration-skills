# Aliyun Bailian China Mainland Chat Transport

Use this file for text-first direct-model requests to the China Mainland deployment.

## Surface Choice

Use `responses` for new `qwen3.7-plus` agent flows that need provider-managed continuation, enum-style reasoning effort, Session Cache, provider-hosted tools, or Responses event streaming.

Use `chat-completions` for compatibility flows that need manual message history, `enable_thinking`, `thinking_budget`, `preserve_thinking`, or the verified non-thinking `json_object` path.
...
- reuse it only with the same provider, China Mainland deployment, connection profile, base URL, API surface, and compatible model family
...
For `qwen3.7-plus`, the following exact built-in types are verified in the official Responses example:
...
| `web_extractor` | `verified` | none documented for `qwen3.7-plus` | must be supplied together with `web_search` |
| `code_interpreter` | `verified` | none documented for `qwen3.7-plus` | normalize code, logs, and container metadata |
...
For `qwen3.7-plus`, `preserve_thinking = true` includes historical assistant `reasoning_content` in the next input and therefore in input-token billing. Do not enable it by default.
...
For `qwen3.7-plus`, `json_object` is verified only in non-thinking mode. A caller requesting it must explicitly set `ThinkingRequested = false`. `json_schema` remains blocked.
