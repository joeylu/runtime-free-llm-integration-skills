...
For `qwen3.8-max`, omit `thinking_budget` whenever `reasoning_effort` is present. For `qwen3.7-plus`, omit `reasoning_effort` and use `enable_thinking` plus optional `thinking_budget`.
