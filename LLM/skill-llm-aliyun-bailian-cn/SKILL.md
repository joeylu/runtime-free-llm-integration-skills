...
description: Standardize Aliyun Bailian China Mainland direct-model integration for chat, vision, imaging, and music across Unity, websites, apps, and backends. Use when Codex needs to choose a China Mainland Aliyun Bailian model, resolve the mainland base URL and request URL, build direct-model request/response flows, wire stream or non-stream transport, expose thinking and temperature controls, build model selectors, or present shared progress and error states. Do not use for Aliyun Bailian International/Singapore, Agent App orchestration, non-Aliyun providers, or prompt-only work with no integration change.
---
...
Own the provider-specific part of Aliyun Bailian China Mainland direct-model integration while reusing the shared contracts under `../_shared`.

Keep this skill focused on:

- Aliyun Bailian China Mainland model catalog
- Aliyun Bailian China Mainland request URLs
- Aliyun Bailian China Mainland model sync workflow
- Aliyun Bailian China Mainland capability verification
- Aliyun Bailian transport rules by model type
- Aliyun-specific logging additions

Do not make this skill own:

- Aliyun Bailian International or Singapore-region access
- Agent App flows
...
Then read these Aliyun China Mainland-specific files:
...
- Treat this skill as Aliyun Bailian China Mainland `direct-model` only. Do not silently switch to International/Singapore or Agent App.
...
- When sync is explicitly requested, collect model rows, pricing, capabilities, context window, max input tokens, and max output tokens live from official Aliyun China Mainland docs by LLM review only. Do not use scripts, scrapers, SDK enum dumps, or automated catalog generators.
...
- Use `references/pricing-matrix.md` for China Mainland billing region, currency, context band, metered side, and unit price. Do not reconstruct tiered pricing from catalog notes.
...
1. Confirm that the task is Aliyun Bailian China Mainland direct-model access.
...
- `references/model-catalog.md` is the default local source for China Mainland model names, labels, and pricing notes.
- `references/pricing-matrix.md` is the structured source for China Mainland region, currency, context-band, and unit-price rows.
