...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `build` | `MiniMax CN Build` | `minimax-cn` | `build` | `template` | `official` | `https://api.minimaxi.com/v1` | `MINIMAX_CN_BUILD_API_KEY` | `env` | `chat,vision,imaging,music` | `chat-completions,image-generation,music-generation` | `chat, vision, imaging, and music endpoints; video HTTP endpoint is reference-only until shared RequestKind adds video` | `2026-07-14` | `intended for implementation or production-like execution flows` |
| `plan` | `MiniMax CN Plan` | `minimax-cn` | `plan` | `template` | `official` | `https://api.minimaxi.com/v1` | `MINIMAX_CN_PLAN_API_KEY` | `env` | `chat` | `chat-completions` | `chat only; image, music, and video access is not verified for this profile` | `2026-07-14` | `intended for planning, review, and analysis flows` |
