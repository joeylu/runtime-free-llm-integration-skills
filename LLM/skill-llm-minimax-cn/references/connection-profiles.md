...
Use this file when the host project defines named MiniMax China Mainland connection profiles.
...
- Do not reuse this profile for MiniMax International.

## Build And Plan Profiles

`build` and `plan` are separate purpose and credential boundaries.

They currently share the same official MiniMax China Mainland base URL, but this is still profile data and must be resolved from the selected row. Do not hardcode the base URL after seeing that both rows match.
