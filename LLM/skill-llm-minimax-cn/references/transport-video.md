# MiniMax China Mainland MiniMax-H3 Video Transport
...
All routes use `Authorization: Bearer <API_KEY>` and `Content-Type: application/json` on `https://api.minimaxi.com`. The account/key must have MiniMax-H3 pay-as-you-go API access. Never switch region, root, API key, surface, model, or billing path after resolution.
...
POST `https://api.minimaxi.com/v2/video_generation` with exactly:
...
- `ProviderOptions.MiniMax.AigcWatermark` may map to optional `aigc_watermark`; default `false`.
...
POST `https://api.minimaxi.com/v2/h3_context_ir` with `model`, validated `content`, required integer `duration` 4–15, mode-valid `ratio`, and optional `callback_url`.
...
POST `https://api.minimaxi.com/v2/video_regeneration`. Resolve a strict exclusive union:
...
| --- | --- | --- |
| Query one | `GET https://api.minimaxi.com/v2/query/video_generation/{task_id}` | only tasks still available in the latest 7-day window |
| List | `GET https://api.minimaxi.com/v2/query/video_generation` | latest 7 days; optional documented filters only |
| Cancel/delete | `DELETE https://api.minimaxi.com/v2/video_generation/{task_id}` | state-dependent behavior below |
