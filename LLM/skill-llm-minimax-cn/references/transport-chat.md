...
`ConnectionProfileKey` must be either `build` or `plan` unless the host project explicitly defines another MiniMax China Mainland profile. Even when both profiles use `https://api.minimaxi.com/v1`, resolve `ResolvedBaseUrl` from the selected profile and use that profile's API key reference.

## Request Mapping

MiniMax China Mainland chat uses the OpenAI-compatible Chat Completions request shape.
...
- `config_error` when `MINIMAX_CN_BUILD_API_KEY`, `MINIMAX_CN_PLAN_API_KEY`, or the selected profile is missing
