...
| Field | Meaning |
| --- | --- |
| `RequestKind` | canonical kind: `text-chat`, `multimodal-chat`, `image-generation`, `speech-generation`, `transcription`, `realtime-audio`, `music-generation`, or `video-generation`; legacy aliases are accepted only at the normalization boundary |
| `ConnectionProfileKey` | named connection profile such as `build` or `plan`; resolved before model selection |
| `ApiVersion` | exact API version selected with the surface, such as `v1`, `v1beta`, `provider-default`, or `none` |
...
| `Instructions` | optional top-level system/developer instruction when the selected surface documents a separate instruction field |
| `ThinkingRequested` | caller intent to request reasoning; when omitted, the selected model-and-surface default applies |
...
| `MaxOutputTokens` | optional output-token limit, mapped only when the selected transport verifies the wire field and range |
...
| `CacheKey` | optional provider cache-routing key when the selected surface verifies one |
| `CacheRetention` | optional provider cache-retention value when the selected surface verifies one |
...
| --- | --- | --- |
| `text-chat` | `Messages` or a provider-verified direct text input | none |
| `multimodal-chat` | `Messages` plus media items required by the selected transport | `Images`, `Audio`, `Video`, `Files` |
| `image-generation` | `Prompt` | `ReferenceImages`, `Seed`, `ImageSize`, `ImageCount` |
| `speech-generation` | provider-verified text input | `Voice`, `AudioFormat`, `Speed` |
| `transcription` | audio input | `Language`, `Prompt`, `Timestamps` |
| `realtime-audio` | provider-verified realtime session input | provider-specific typed options |
| `music-generation` | fields required by the selected provider's conditional matrix | `Prompt`, `Lyrics`, `DurationSeconds`, `Seed`, `IsInstrumental` |
| `video-generation` | provider-verified prompt/reference input | `ReferenceImages`, `DurationSeconds`, `Resolution`, `Seed` |
...
For `multimodal-chat`, `Inputs.Images` is the input image list. Do not use `Inputs.ImageCount` for vision input images; `Inputs.ImageCount` is reserved for image-generation output count.

`ThinkingRequested` is provider-neutral intent. `ReasoningEffort` is an enum-style control whose exact allowed values and wire mapping come from the selected `RequestKind + Model + ApiSurface + ApiVersion` capability row and transport.
...
Normalize legacy names once at the repository boundary:
...
Provider transports and new matrices should use canonical kinds. Do not silently reinterpret one canonical kind as another.
...
- If the selected surface requires signed thought items, encrypted reasoning items, function-call IDs, or other continuation state, preserve them in `ReasoningItems` or typed `ProviderOptions` as directed by the provider transport.
...
- Do not send `CacheKey` or `CacheRetention` unless the selected transport documents the exact wire mapping.
...
- Resolve `ApiSurface`, `ApiVersion`, `ResolvedBaseUrl`, and `ResolvedRequestUrl` before capability lookup and before sending.
- Resolve capabilities by the exact `RequestKind + Model + ApiSurface + ApiVersion` tuple.
